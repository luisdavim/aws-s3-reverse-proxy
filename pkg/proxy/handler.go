...
var (
	...
	// - gra => pass (ceph style)
	awsAuthorizationCredentialRegexp    = regexp.MustCompile("Credential=([a-zA-Z0-9]+)/[0-9]+/([a-zA-Z-0-9]+)/s3/aws4_request")
...
func (h *Handler) validateSignature(req, proxyReq *http.Request) error {
	...
	fakeAuthorizationStr := proxyReq.Header.Get("Authorization")
